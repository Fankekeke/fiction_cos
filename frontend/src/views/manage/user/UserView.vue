    ...
    <div style="font-size: 13px;font-family: SimHei" v-if="authorData !== null">
      ...
      <a-row style="padding-left: 24px;padding-right: 24px;">
        <a-col :span="8"><b>创建时间：</b>
          {{ authorData.createDate }}
        </a-col>
        <a-col :span="8"><b>性别：</b>
          <span v-if="authorData.sex == 1">男</span>
          <span v-if="authorData.sex == 2">女</span>
        </a-col>
      </a-row>
      <br/>
      <a-row style="padding-left: 24px;padding-right: 24px;">
...
export default {
  ...
  watch: {
    authorShow: function (value) {
      if (value) {
        if (this.authorData.images) {
          this.imagesInit(this.authorData.images)
        }
      }
    }
  },
  methods: {
    local (authorData) {
      ...
      let point = new BMap.Point(authorData.longitude, authorData.latitude)
