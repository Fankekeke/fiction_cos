<template>
  <a-modal v-model="show" title="修改教师" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        ...
        <a-col :span="24">
          <a-form-item label='教师头像' v-bind="formItemLayout">
            <a-upload
              name="avatar"
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...teacher}) {
      ...
      let fields = ['name', 'sex', 'birthday', 'phone', 'content', 'status']
      let obj = {}
      Object.keys(teacher).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(teacher['images'])
        }
        if (key === 'sex' || key === 'status') {
          teacher[key] = teacher[key].toString()
        }
        if (key === 'birthday') {
          if (key === 'birthday' && teacher[key] != null) {
            teacher[key] = moment(teacher[key])
          }
        }
