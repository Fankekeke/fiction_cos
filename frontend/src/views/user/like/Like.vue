...
import recordView from './LikeView.vue'
...
export default {
    ...
    columns () {
      return [ {
        ...
        customRender: (text, record, index) => {
          ...
          return <a-popover>
            <template slot="content">
              <a-avatar shape="square" size={132} icon="user" src={ 'http://127.0.0.1:9527/imagesWeb/' + record.authorImages.split(',')[0] } />
            </template>
            <a-avatar shape="square" icon="user" src={ 'http://127.0.0.1:9527/imagesWeb/' + record.authorImages.split(',')[0] } />
          </a-popover>
        }
      }, {
        title: '点赞时间',
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/book-like-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/book-like-info/page', {
        ...params
      }).then((r) => {
